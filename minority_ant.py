        ...
        if self.prioritize_pheromone_routes:
            ...
            for i in range(edge_values.shape[0]):
                if edge_values[i] < 0.01:
                    filteres_targets.append(i)
            if len(filteres_targets) > 0:
                edge_values = np.delete(edge_values, filteres_targets)
                target_nodes = np.delete(target_nodes, filteres_targets)
